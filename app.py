...
def calculer_score_ameliore(row, df_global, df_course):
    score = 0
    cheval_nom = str(row.get('Cheval', '')).strip()
    dist_actuelle = row.get('Dist', 0)
    jockey_actuel = str(row.get('Jockey', '')).strip().upper()
    entraineur_actuel = str(row.get('Entraîneur', '')).strip().upper()
    
    # Récupérer tout l'historique de ce cheval
    hist_cheval = df_global[df_global['Cheval'] == cheval_nom]
    ...
    if chiffres:
        ...
        score += 10 # Note neutre si pas de musique

    # ==========================================
    # 2. COUPLE JOCKEY / ENTRAÎNEUR (25 pts)
    # ==========================================
    # Victoires avec ce Jockey (5 pts par victoire, max 12,5)
    victoires_jockey = len(hist_cheval[
        (hist_cheval['Jockey'].astype(str).str.strip().str.upper() == jockey_actuel) & 
        (hist_cheval['Classement'] == 1)
    ]) if pd.notna(jockey_actuel) and jockey_actuel != "" else 0
    score_jockey = min(12.5, victoires_jockey * 5)
    
    # Victoires avec cet Entraîneur (5 pts par victoire, max 12,5)
    victoires_entraineur = len(hist_cheval[
        (hist_cheval['Entraîneur'].astype(str).str.strip().str.upper() == entraineur_actuel) & 
        (hist_cheval['Classement'] == 1)
    ]) if pd.notna(entraineur_actuel) and entraineur_actuel != "" else 0
    score_entraineur = min(12.5, victoires_entraineur * 5)
    
    score += score_jockey + score_entraineur

    # ==========================================
    # 3. AFFINITÉ DISTANCE (15 pts)
    # ==========================================
    # On cherche les performances sur la distance actuelle ± 200 mètres
    ...
    # 10 pts pour une victoire, 3 pts pour un podium (max 15)
    score_distance = min(15, (victoires_dist * 10) + (podiums_dist * 3))
...
st.markdown("---")
>>>>>>> dfcd05259fe718ca8f0ad2926662b66b525719ed
